...
export default class Mahjong_page extends Page {
  ...
  async makeLeaveButton(_ref) {
    ...
    document
      ...
      .appendChild(this.makeElement("button", { id: "leave" }));
    INSTANCES[FB_IO_INSTANCE].FB_Listener(
      `${this.#currentLobby}/hands/${this.#currentPlayer}`,
      this.displayHand.bind(this),
    );
    INSTANCES[FB_IO_INSTANCE].FB_Listener(
      `${this.#currentLobby}/wins`,
      this.manageWin.bind(this),
    );
    INSTANCES[FB_IO_INSTANCE].FB_Listener(
      `${this.#currentLobby}/discards`,
      this.displayDiscards.bind(this),
    );
    INSTANCES[FB_IO_INSTANCE].FB_Listener(
      `${this.#currentLobby}/calls`,
      this.displayDiscards.bind(this),
    );
    INSTANCES[FB_IO_INSTANCE].FB_Listener(
      `${this.#currentLobby}/turn`,
      this.displayDiscards.bind(this),
    );
    let calls = await INSTANCES[FB_IO_INSTANCE].FB_Read(
      `${this.#currentLobby}/calls/${this.#currentPlayer}`,
    );
    ...
    if (!isNaN(_ref.slice(-1)) && this.#listenerIsOn == false) {
      ...
      INSTANCES[FB_IO_INSTANCE].FB_Listener(
        this.#currentLobby,
        this.updatePlayerCount.bind(this),
      );
    }
  }
  ...
  //
  //input _join
  //=whether to join a lobby if they are not in one
  //
  ...
  async lobbyCheck(_join) {
    ...
    let lobby;
    for (let i = 1; i < 5; i++) {
      let ref = await INSTANCES[FB_IO_INSTANCE].FB_Finder(
        "/lobbies/mahjong/",
        1,
        `players/player${i}`,
        UID,
      ...
      if (refval != null) {
        j++;
        lobby = `/lobbies/mahjong/${Object.keys(refval)[0]}/players/player${i}`;
    ...
    if (j < 1 && this.#isInLobby == false) {
      if (_join == true) {
        ...
        this.joinLobby(UID);
        return true;
      } else this.#isInLobby = false;
      return false;
    } else this.#isInLobby = true;
    return lobby;
  }
  ...
  async joinLobby(UID) {
    ...
    if (ref.val() == null) {
      ...
      await INSTANCES[FB_IO_INSTANCE].FB_Write(
        `/lobbies/mahjong/lobby${UID}${D}`,
        { players: { player1: UID }, open: "true" },
      );
      this.makeLeaveButton(`/lobbies/mahjong/lobby${UID}${D}/players/player1`);
    } else {
      let j = 0;
      let lobby = await INSTANCES[FB_IO_INSTANCE].FB_Read(
        `/lobbies/mahjong/${Object.keys(ref.val())[0]}/players/`,
      );
      for (let i = 1; i < 5; i++) {
        if (lobby != null) {
          if (lobby[`player${i}`] == undefined) {
            ...
            this.makeLeaveButton(
              `/lobbies/mahjong/${Object.keys(ref.val())[0]}/players/player${i}`,
            );
            break;
          } else if (i == 4) {
            INSTANCES[FB_IO_INSTANCE].FB_Write(
              `/lobbies/mahjong/${Object.keys(ref.val())[0]}/`,
              { open: "pending" },
            );
            this.joinLobby(UID);
          }
        } else {
          INSTANCES[FB_IO_INSTANCE].FB_Remove(
            `/lobbies/mahjong/${Object.keys(ref.val())[0]}`,
          );
          j++;
        }
  ...
  /*****************************************************/
  //updatePlayerCount(_ref)
  //
  //input _ref
  //=the current players lobby
  //
  //updates the player count,
  //if player one makes them start game on 4 players
  /*****************************************************/
  async updatePlayerCount(_ref) {
    if (_ref == null) return;
    if (_ref["players"] == null) return;
    //console.log(_ref,this.#currentPlayer)
    if (
      INSTANCES[FB_IO_INSTANCE].getUID() == _ref["players"][this.#currentPlayer]
    ) {
      document.getElementById("waitCount").innerHTML =
        Object.keys(_ref["players"]).length + " players in current lobby";
    } else {
      document.getElementById("waitCount").innerHTML =
        "0 players in current lobby";
    }
    if (INSTANCES[FB_IO_INSTANCE].getUID() == _ref["players"][`player1`]) {
      if (
        Object.keys(_ref["players"]).length == 4 &&
        (_ref["open"] == "true" || _ref["open"] == "pending")
      ) {
        INSTANCES[FB_IO_INSTANCE].FB_Write(this.#currentLobby, {
          open: "false",
        });
        this.createGame();
      }
  ...
  //displayHand()
  //
  //displays the tiles in the users hand
  /*****************************************************/
  async displayHand() {
    if (document.getElementById("handDiv")) {
      document.querySelectorAll("#handDiv").forEach((_el) => _el.remove());
    }
    let val = await INSTANCES[FB_IO_INSTANCE].FB_Read(
      `${this.#currentLobby}/hands/${this.#currentPlayer}`,
    );
    let handtiles = [];
    if (val == null) return;
    if (Object.values(val).length >= 13) this.#callCount = 0;
    let pla = await INSTANCES[FB_IO_INSTANCE].FB_Read(
      `${this.#currentLobby}/playOrder`,
    );
    this.#playOrder = { playOrder: pla };
    const POSITION = Object.keys(this.#playOrder["playOrder"]).find(
      (POSITION) =>
        this.#playOrder["playOrder"][POSITION] === this.#currentPlayer,
    );
    //document.getElementById('position').innerHTML = POSITION
    for (let i = 1; i < 5; i++) {
      await INSTANCES[FB_IO_INSTANCE].FB_DestroyListener(
        `${this.#currentLobby}/skips/${i}`,
      );
    }
    INSTANCES[FB_IO_INSTANCE].FB_Listener(
      `${this.#currentLobby}/skips/${POSITION}`,
      this.manageSkips.bind(this),
    );
    val.forEach((_tile) => {
      handtiles.push(
        this.makeElement(
    ...
    document
      .getElementById("hand")
      .replaceChildren(this.makeElement("div", { id: "handDiv" }, handtiles));
    document.querySelectorAll(".tile").forEach(async (_el) => {
      _el.addEventListener("click", (e) => {
        this.discard(e, val);
      });
      if (![14, 11, 8, 5, 2].includes(val.length)) {
    ...
    if (correctLength == true) {
      let waits = await INSTANCES[FB_IO_INSTANCE].FB_Read(
        `${this.#currentLobby}/waits/${this.#currentPlayer}/waits`,
      );
      //console.log(waits)
      //console.log(this.#callCount)
      if (waits != false) {
        if (waits.includes(val[val.length - 1]))
          this.makeTsumoButton(val[val.length - 1]);
      }
    }
  ...
  /*****************************************************/
  //discard(_tile,_currentHand)
  //
  //input _tile
  //=the element of the tile to discard
  //input _currentHand
  //=the array of the entire hand (reasons)
  //
  //discards tiles
  /*****************************************************/
  async discard(_tile, _currentHand) {
    let val = _tile["target"].getAttribute("data-value");
    this.#hasDiscarded = true;
    let d = new Date();
    let D = d.getTime();
    for (let i = 0; i < _currentHand.length; i++) {
      if (_currentHand[i] == val) {
        _currentHand.splice(i, 1);
        break;
      }
    }
    await INSTANCES[FB_IO_INSTANCE].FB_Set(
      `${this.#currentLobby}/hands/${this.#currentPlayer}`,
      _currentHand.sort(),
    );
    await INSTANCES[FB_IO_INSTANCE].FB_Write(
    ...
    _tile["target"].remove();
    this.checkWaits(val);
    let waits = await this.manageHand(_currentHand.sort());
    //console.log('bronchitis',_currentHand,waits)
    await INSTANCES[FB_IO_INSTANCE].FB_Write(
      `${this.#currentLobby}/waits/${this.#currentPlayer}`,
      waits,
    );
    //this.displayHand()
  }
  ...
  //
  //input _tile
  //=the tile to check waits for
  //
  //checks if anyone can call the tile just discarded
  /*****************************************************/
  async checkWaits(_tile) {
    let skips = { 1: true, 2: true, 3: true, 4: true };
    let waits = await INSTANCES[FB_IO_INSTANCE].FB_Read(
      `${this.#currentLobby}/waits/`,
    );
    //console.log(waits)
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (
        waits[this.#playOrder["playOrder"][otherPlayers]].ponWaits?.includes(
          _tile.slice(0, 2),
        )
      )
        skips[otherPlayers] = false;
      if (
        waits[this.#playOrder["playOrder"][otherPlayers]].kanWaits?.includes(
          _tile.slice(0, 2),
        )
      )
        skips[otherPlayers] = false;
      if (waits[this.#playOrder["playOrder"][otherPlayers]].waits != false) {
        if (
          waits[this.#playOrder["playOrder"][otherPlayers]].waits?.includes(
            _tile.slice(0, 2),
          )
        )
          skips[otherPlayers] = false;
    ...
    if (
      waits[this.#playOrder["playOrder"][nextPlayer]].chiWaits?.includes(
        _tile.slice(0, 2),
      )
    )
      skips[nextPlayer] = false;
    INSTANCES[FB_IO_INSTANCE].FB_Write(this.#currentLobby, { skips: skips });
    INSTANCES[FB_IO_INSTANCE].FB_Listener(
      `${this.#currentLobby}/skips/`,
      this.skipWaits.bind(this),
    );
  }
  ...
  //
  //input _skips
  //=the object saying how each player responds
  //
  //handles what to do with a discard,
  //wheather it's skipped or not
  /*****************************************************/
  skipWaits(_skips) {
    if (_skips.includes(false) == false) {
      INSTANCES[FB_IO_INSTANCE].FB_DestroyListener(
        `${this.#currentLobby}/skips`,
      );
  ...
  //pickUp()
  //
  //removes a tile from the deck and
  //adds it to the next players hand
  /*****************************************************/
  async pickUp() {
    const POSITION = Object.keys(this.#playOrder["playOrder"]).find(
      (POSITION) =>
    ...
    let tile = await INSTANCES[FB_IO_INSTANCE].FB_Read(
      `${this.#currentLobby}/deck/`,
    );
    let drawn = tile[tile.length - 1];
    let nextHand = await INSTANCES[FB_IO_INSTANCE].FB_Read(
      `${this.#currentLobby}/hands/${this.#playOrder["playOrder"][nextPlayer]}`,
    );
    INSTANCES[FB_IO_INSTANCE].FB_Remove(
      `${this.#currentLobby}/deck/${tile.length - 1}`,
    );
    await INSTANCES[FB_IO_INSTANCE].FB_Write(
      `${this.#currentLobby}/hands/${this.#playOrder["playOrder"][nextPlayer]}`,
      { [nextHand.length]: drawn },
    );
    //this.displayHand()
  }
  ...
  //
  //input _val
  //=Object with the users response to the possible call
  //
  //makes the button for the skips and steals
  /*****************************************************/
    ...
    if (_val == false && !document.getElementById("skip")) {
      ...
      if (!document.getElementById("skip")) {
        ...
        document.getElementById("skip").onclick = () => {
          ...
          INSTANCES[FB_IO_INSTANCE].FB_Write(`${this.#currentLobby}/skips/`, {
            [POSITION]: true,
          });
        };
        let turn = await INSTANCES[FB_IO_INSTANCE].FB_Read(
          `${this.#currentLobby}/turn`,
        );
        let temp = await INSTANCES[FB_IO_INSTANCE].FB_SortedRead(
          `${this.#currentLobby}/discards/${this.#playOrder["playOrder"][turn]}`,
          false,
          1,
          false,
        );
        ...
        if (this.#playOrder["playOrder"][turnOverlap] == this.#currentPlayer) {
          if (
            waits.chiWaits?.includes(
              Object.values(currentDiscard)[0].slice(0, 2),
            )
          )
            this.makeStealButton(currentDiscard, "chi", turn);
        }
        if (
          waits.kanWaits?.includes(Object.values(currentDiscard)[0].slice(0, 2))
        )
          this.makeStealButton(currentDiscard, "kan", turn);
        if (
          waits.ponWaits?.includes(Object.values(currentDiscard)[0].slice(0, 2))
        )
          this.makeStealButton(currentDiscard, "pon", turn);
        if (waits.waits != false) {
          if (
            waits.waits?.includes(Object.values(currentDiscard)[0].slice(0, 2))
          )
            this.makeStealButton(currentDiscard, "ron", turn);
        }
      }
    }
  }
  ...
  /*****************************************************/
  //makeTsumoButton(_tile)
  //
  //input _tile
  //=the tile to tsumo on
  //
  //makes the button for tsumo
  /*****************************************************/
  async makeTsumoButton(_tile) {
    document
      .querySelectorAll(".tile")
      .forEach((_el) => _el.setAttribute("disabled", true));
    if (
      (await INSTANCES[FB_IO_INSTANCE].FB_Read(
        `${this.#currentLobby}/wins/tsumo`,
      )) == this.#currentPlayer
    )
      return;
    let el = await this.makeElement("button", { id: "skip" });
    await document.getElementById("stealIndicator").append(el);
    document.getElementById("skip").innerHTML = "skip";
    let el2 = await this.makeElement("button", {
      id: "tsumo",
      "data-value": _tile,
    });
    ...
    el2.onclick = async () => {
      ...
      el2.remove();
      INSTANCES[FB_IO_INSTANCE].FB_Write(this.#currentLobby, {
        wins: { tsumo: this.#currentPlayer },
      });
      let UID = INSTANCES[FB_IO_INSTANCE].getUID();
      let userwins = await INSTANCES[FB_IO_INSTANCE].FB_Read(
        `leaderboards/mahjong/${UID}`,
      );
      //console.log(userwins)
      if (userwins == null) userwins = { wins: 0 };
      if (!isNaN(userwins["wins"]))
        userwins["wins"] = Number(userwins["wins"]) + 1;
  ...
  //
  //input _tile
  //=an object containing the tile to steal
  //input _type
  //=the type of steal
  //input _from
  //which player to steal it from
  //
  //makes the steal button
  /*****************************************************/
  async makeStealButton(_tile = {}, _type, _from) {
    ...
    await document.getElementById("stealIndicator").append(
      await this.makeElement("button", {
        id: _type,
        "data-value": Object.values(_tile)[0],
        class: "steal",
        from: _from,
      }),
    );
  ...
  //
  //input _val
  //=the object containing each players
  //response to the discard
  //
  //manages the stealand wheather you have priority
  /*****************************************************/
  async steal(_val) {
    let turn = await INSTANCES[FB_IO_INSTANCE].FB_Read(
      `${this.#currentLobby}/turn`,
    ...
    if (_val.includes(false) == false) {
      ...
      if (typeval >= enemtypeval) {
        ...
        let hand = await INSTANCES[FB_IO_INSTANCE].FB_Read(
          `${this.#currentLobby}/hands/${this.#currentPlayer}`,
        );
        INSTANCES[FB_IO_INSTANCE].FB_Write(`${this.#currentLobby}/skips/`, {
          [POSITION]: _type,
          ...
          if (chi.length == 1) {
            ...
            chi.forEach((_set) => {
              possibleChi.push(
                this.makeElement(
                  "button",
                  { class: "chiOptions", "data-value": JSON.stringify(_set) },
                  [
                    this.makeElement("img", {
                      src: `./mahjong_tiles/${_set[0]}.png`,
                    }),
                    this.makeElement("img", {
                      src: `./mahjong_tiles/${_set[1]}.png`,
                    }),
                  ],
                ),
              );
            ...
            document
              .getElementById("stealIndicator")
              .append(
                this.makeElement("div", { id: "chiOptionDiv" }, possibleChi),
              );
            document.querySelectorAll(".chiOptions").forEach((_el) => {
              _el.onclick = async () => {
                ...
                document
                  .querySelectorAll(".chiOptions")
                  .forEach((_div) => _div.remove());
        ...
        if (_type == "pon") {
          let pon = [
            Object.values(_tile)[0].slice(0, 2),
            Object.values(_tile)[0].slice(0, 2),
          ];
        ...
        if (_type == "ron") {
          INSTANCES[FB_IO_INSTANCE].FB_Write(this.#currentLobby, {
            wins: { [POSITION]: this.#currentPlayer },
          });
          let UID = INSTANCES[FB_IO_INSTANCE].getUID();
          let userwins = await INSTANCES[FB_IO_INSTANCE].FB_Read(
            `leaderboards/mahjong/${UID}`,
  ...
  //response to the discard
  //
  //alerts users who won the round
  /*****************************************************/
  async manageWin(_val) {
    let winners = [];
    if (_val == null || _val == undefined) return;
    for (let _player of Object.values(_val)) {
      let winnerUID = await INSTANCES[FB_IO_INSTANCE].FB_Read(
        `${this.#currentLobby}/players/${_player}`,
      );
      let winnerName = await INSTANCES[FB_IO_INSTANCE].FB_Read(
        `users/${winnerUID}/public/username`,
      );
      if (winnerName != null) {
    ...
    for (let i = 1; i < winners.length; i++)
      winnerString = winnerString + " and " + winners[i];
    if (winners.length != 0) {
  ...
  //kanDraw()
  //
  //makes the player pick up from the deadwall after a kan
  /*****************************************************/
  async kanDraw() {
    let kanCount = await INSTANCES[FB_IO_INSTANCE].FB_Read(
      `${this.#currentLobby}/kanCount`,
    );
    kanCount++;
    let kanDraw = await INSTANCES[FB_IO_INSTANCE].FB_Read(
      `${this.#currentLobby}/deadwall/kan/${kanCount}`,
    );
    let hand = await INSTANCES[FB_IO_INSTANCE].FB_Read(
      `${this.#currentLobby}/hands/${this.#currentPlayer}`,
    );
  ...
  //
  //input _tile
  //=the tile that can be stolen
  //
  //checks which sequences can go with the stolen tile
  /*****************************************************/
  async checkChi(_tile) {
    ...
    let blandFives = [];
    //console.log(_tile)
    let shand = await INSTANCES[FB_IO_INSTANCE].FB_Read(
      `${this.#currentLobby}/hands/${this.#currentPlayer}`,
    );
    ...
    if (hand.includes(_tile.slice(0, 1) + (Number(_tile.slice(1, 2)) + 1))) {
      if (hand.includes(_tile.slice(0, 1) + (Number(_tile.slice(1, 2)) + 2))) {
        viable.push([
          _tile.slice(0, 1) + (Number(_tile.slice(1, 2)) + 1),
          _tile.slice(0, 1) + (Number(_tile.slice(1, 2)) + 2),
        ]);
      }
      if (hand.includes(_tile.slice(0, 1) + (Number(_tile.slice(1, 2)) - 1))) {
        viable.push([
    ...
    redFives.forEach((_tile) => {
      for (let i = 0; i < viable.length; i++) {
        const POSITION = Object.keys(viable[i]).find(
          (POSITION) => viable[i][POSITION] === _tile.slice(0, 2),
        );
        if (POSITION !== undefined) {
          if (blandFives.includes(_tile.slice(0, 2)))
            viable.push([...viable[i]]);
  ...
  async displayDiscards() {
    ...
    let discards = await INSTANCES[FB_IO_INSTANCE].FB_Read(
      `${this.#currentLobby}/discards`,
    );
    let discard = [];
    let discard2 = {};
    for (let i = 0; i < 4; i++) {
      ...
      if (discards != null) {
        if (discards[this.#playOrder["playOrder"][currentPos]] != null) {
          Object.values(
            discards[this.#playOrder["playOrder"][currentPos]],
          )?.forEach((_tile) => {
            arr.push(
              this.makeElement("img", { src: `./mahjong_tiles/${_tile}.png` }),
            );
          });
        }
      }
      ...
      discard.push(
        this.makeElement("div", { class: "discardsjr", id: call }),
        this.makeElement(
          "div",
          { class: "discardsjr", id: `discards${i}` },
          arr,
        ),
      );
    }
    discard.push(
      this.makeElement("div", { class: "discardsjr", id: "calls2" }),
    );
    let temp = discard[5];
    discard[5] = discard[7];
    discard[7] = temp;
    if (document.querySelector(".discards")) {
      ...
      document
        .getElementById("discardDiv")
        .append(this.makeElement("div", { class: "discards" }));
      document.querySelector(".discards").append(...discard);
    }
    document
      .getElementById("discards1")
      .childNodes.forEach((_el) => (_el.style.rotate = "90deg"));
    document
      .getElementById("discards3")
      .childNodes.forEach((_el) => (_el.style.rotate = "90deg"));
  ...
  //displayCenter()
  //
  //displays the center of the board holding turn
  //indicators and seat winds
  /*******************************************************/
  async displayCenter() {
    const POSITION = Object.keys(this.#playOrder["playOrder"]).find(
      (POSITION) =>
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (i == 3) call = `wind1`;
      discard.push(
        this.makeElement("div", { class: "central", id: call }),
    ...
    if (!document.querySelector(".centralDiv")) {
      ...
      document
        ...
        .replaceChildren(
          this.makeElement("div", { class: "centralDiv" }, discard),
        );
    }
    ...
    Object.keys(this.#playOrder["playOrder"]).forEach(async (_pos) => {
      let player = await INSTANCES[FB_IO_INSTANCE].FB_Read(
        `${this.#currentLobby}/players/${this.#playOrder["playOrder"][_pos]}`,
      );
      pfp[_pos] = await INSTANCES[FB_IO_INSTANCE].FB_Read(
        `/users/${player}/publicFixed/PhotoURL`,
      );
      ...
      document
        .getElementById(`wind${Number(post)}`)
        .replaceChildren(
      ...
      if (
        ...
        if (!document.getElementById(`turnIndicator${post}`)) {
          document
            .getElementById(`central${Number(post)}`)
            .replaceChildren(
              this.makeElement("div", {
                class: "turnIndicatorOn",
                id: `turnIndicator${post}`,
              }),
            );
        } else
          document
            .getElementById(`turnIndicator${post}`)
            .setAttribute("class", "turnIndicatorOn");
      } else {
        ...
        if (!document.getElementById(`turnIndicator${post}`)) {
          ...
          document
            .getElementById(`turnIndicator${post}`)
            .setAttribute("class", "turnIndicatorOff");
      }
    });
  }
  ...
  //displayCalls()
  //
  //displays what tiles have being called
  /*****************************************************/
  async displayCalls() {
    const POSITION = Object.keys(this.#playOrder["playOrder"]).find(
      (POSITION) =>
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (calls != null) {
        if (calls[this.#playOrder["playOrder"][currentPos]] != undefined) {
          Object.values(
            calls[this.#playOrder["playOrder"][currentPos]],
          )?.forEach((_set) => {
            let call = [];
  ...
  //createGame()
  //
  //creates the game by dealing and other stuff
  /*****************************************************/
  async createGame() {
    let playOrder = this.shuffleDeck([
      "player1",
      "player2",
      "player3",
      "player4",
  ...
  calculateKanWaits(_ponWaits) {
    ...
    if (_ponWaits.length > 1) {
      for (let i = 0; i < _ponWaits.length - 1; i++) {
        if (_ponWaits[i] == _ponWaits[i + 1].slice(0, 2)) {
  ...
  calculateChiWaits(_hand) {
    ...
    for (let i = 0; i < _hand.length - 1; i++) {
      if (_hand[i].slice(0, 1) == _hand[i + 1].slice(0, 1)) {
        if (
          Number(_hand[i].slice(1, 2)) + 1 ==
          Number(_hand[i + 1].slice(1, 2))
        ) {
          if (_hand[i].slice(1, 2) == "1")
            chiWaits.push(_hand[i].slice(0, 1) + 3);
          else if (_hand[i].slice(1, 2) == "8")
            chiWaits.push(_hand[i].slice(0, 1) + 7);
          else {
            chiWaits.push(
              _hand[i].slice(0, 1) + (Number(_hand[i].slice(1, 2)) - 1),
            );
            chiWaits.push(
              _hand[i].slice(0, 1) + (Number(_hand[i + 1].slice(1, 2)) + 1),
            );
          }
        } else if (
          Number(_hand[i].slice(1, 2)) + 2 ==
          Number(_hand[i + 1].slice(1, 2))
        ) {
          chiWaits.push(
            _hand[i].slice(0, 1) + (Number(_hand[i].slice(1, 2)) + 1),
      ...
      for (let b = 0; b < arr.length; b++) {
        ...
        for (let c = 0; c < arr.length; c++) {
          ...
          for (let d = 0; d < arr.length; d++) {
            ...
            if (poss.includes("m5") && _hand.includes("m5r"))
              poss[poss.indexOf("m5")] = "m5r";
            if (poss.includes("p5") && _hand.includes("p5r"))
              poss[poss.indexOf("p5")] = "p5r";
            if (poss.includes("s5") && _hand.includes("s5r"))
              poss[poss.indexOf("s5")] = "s5r";
          ...
          for (let d = 0; d < _ponWaits.length; d++) {
            ...
            if (this.AsupersetB(_hand, poss)) {
              if (this.AremoveB(_hand, poss).length == 1) {
                ...
                arr2.push(
                  ...this.calculateChiWaits(this.AremoveB(_hand, poss)),
                );
                arr2.push(
                  ...this.calculatePonWaits(this.AremoveB(_hand, poss)),
                );
              }
            }
          }
